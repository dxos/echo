...
export abstract class KeyMapper<T, S> {
  ...
  get (frame: protocol_dxos.echo.Timeframe.IFrame): T {
    return (frame as IIndexable)[this._key];
  }

  toArray (timeframe: protocol_dxos.echo.ITimeframe): [T, number][] {
    const { frames = [] } = timeframe;
    assert(frames);
    return frames.map(frame => [(frame as IIndexable)[this._key], frame.seq as number]);
  }

  fromArray (frames: [T, number][]): protocol_dxos.echo.ITimeframe {
    return {
...
export class Spacetime<T, S> {
  ...
  toJson (timeframe: protocol_dxos.echo.ITimeframe) {
    assert(timeframe);
    const { frames = [] } = timeframe;
  ...
  stringify (timeframe?: protocol_dxos.echo.ITimeframe | null | undefined) {
    return timeframe ? JSON.stringify(this.toJson(timeframe)) : null;
  }

  createTimeframe (frames?: [T, number][]): protocol_dxos.echo.ITimeframe {
    return {
   ...
   * @param timeframes
   */
  merge (...timeframes: protocol_dxos.echo.ITimeframe[]): protocol_dxos.echo.ITimeframe {
    const map = new Map();
   ...
   * @param keys
   */
  removeKeys (timeframe: protocol_dxos.echo.ITimeframe, keys: T[]): protocol_dxos.echo.ITimeframe {
    return {
   ...
   * @param tf2
   */
  dependencies<T> (tf1: protocol_dxos.echo.ITimeframe, tf2: protocol_dxos.echo.ITimeframe): protocol_dxos.echo.ITimeframe {
    return {
