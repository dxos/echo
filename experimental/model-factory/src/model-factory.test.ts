...
import { dxos as prototol_dxos } from '@dxos/experimental-echo-protocol';
...
describe('model factory', () => {
  ...
  test('model mutation processing', async () => {
    ...
    const objects: prototol_dxos.echo.testing.ITestItemMutation[] = [];

    // Transform outbound mutations to inbounds model messsges (create loop).
    const writeStream = createTransform<
      prototol_dxos.echo.testing.ITestItemMutation, ModelMessage<prototol_dxos.echo.testing.ITestItemMutation>
      >(
        async (mutation: prototol_dxos.echo.testing.ITestItemMutation) => {
          objects.push(mutation);
          const out: ModelMessage<prototol_dxos.echo.testing.ITestItemMutation> = {
            meta: {
              feedKey,
              seq: 1
            },
    ...
    expect(objects[0]).toEqual(createAny<prototol_dxos.echo.testing.ITestItemMutation>({
      key: 'title',
      value: 'Hello'
