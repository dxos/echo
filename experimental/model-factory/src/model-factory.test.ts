...
describe('model factory', () => {
  ...
  test('model mutation processing', async () => {
    ...
    const objects: protocol_dxos.echo.testing.ITestItemMutation[] = [];

    // Transform outbound mutations to inbounds model messsges (create loop).
    const writeStream = createTransform<
      protocol_dxos.echo.testing.ITestItemMutation, ModelMessage<protocol_dxos.echo.testing.ITestItemMutation>
      >(
        async (mutation: protocol_dxos.echo.testing.ITestItemMutation) => {
          objects.push(mutation);
          const out: ModelMessage<protocol_dxos.echo.testing.ITestItemMutation> = {
            meta: {
              feedKey,
              seq: 1
            },
    ...
    expect(objects[0]).toEqual(createAny<protocol_dxos.echo.testing.ITestItemMutation>({
      key: 'title',
      value: 'Hello'
