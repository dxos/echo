...
export class KeyValueUtil {
  static createMessage (key: string, value: any): object_dxos.echo.object.IKeyValue {
    assert(key);
...
export class ValueUtil {
   ...
   * @return {{Value}}
   */
  static createMessage (value: any): object_dxos.echo.object.IValue {
  ...
  static bytes (value: Uint8Array): object_dxos.echo.object.IValue {
    return { [Type.BYTES]: value };
  }

  static bool (value: boolean): object_dxos.echo.object.IValue {
    return { [Type.BOOLEAN]: value };
  }

  static integer (value: number): object_dxos.echo.object.IValue {
    return { [Type.INTEGER]: value };
  }

  static float (value: number): object_dxos.echo.object.IValue {
    return { [Type.FLOAT]: value };
  }

  static string (value: string): object_dxos.echo.object.IValue {
    return { [Type.STRING]: value };
  }

  static datetime (value: string): object_dxos.echo.object.IValue {
    return { [Type.DATETIME]: value };
  }

  static object (value: Record<string, any>): object_dxos.echo.object.IValue {
    return {
      [Type.OBJECT]: {
  ...
  static applyValue (object: any, key: string, value: object_dxos.echo.object.IValue) {
    assert(object);
    assert(key);
    assert(value);
    ...
    if (value[Type.OBJECT]) {
      const nestedObject = {};
      const { properties }: { properties: object_dxos.echo.object.KeyValue[] } = value[Type.OBJECT]!;
...
export class MutationUtil {
  static applyMutationSet (object: any, message: object_dxos.echo.object.IObjectMutationSet) {
    assert(message);
  ...
  static applyMutation (object: any, mutation: object_dxos.echo.object.IObjectMutation) {
    const { operation = object_dxos.echo.object.ObjectMutation.Operation.SET, key, value } = mutation;
    switch (operation) {
      // TODO(burdon): Namespace conflict when imported into echo-db.
      case 0: { // object_dxos.echo.object.ObjectMutation.Operation.SET: {
